...
struct Cli {
    ...
    #[structopt(long = "out", short = "o", default_value = "./")]
    out_dir: String,
...
main!(|args: Cli, log_level: verbosity| {
    let extension = &args.file[args.file.len() - 3..];

    if extension != ".tl" {
        eprintln!("You probably want to pass a .tl file.");
        std::process::exit(1);
    }
    ...
    let (tl_functions, tl_constructors) = process_tl_scheme(&mut file);

    create_dir_all(&args.out_dir).unwrap();

    let constructors_path = Path::new(&args.out_dir).join("constructors.rs");
    let cons_ser_path = Path::new(&args.out_dir).join("constructors_serialize.rs");
    let cons_des_path = Path::new(&args.out_dir).join("constructors_deserialize.rs");

    let functions_path = Path::new(&args.out_dir).join("functions.rs");
    let func_ser_path = Path::new(&args.out_dir).join("functions_serialize.rs");
